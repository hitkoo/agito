...
function useSpritePreview(relativePath: string): string | null {
  ...
  useEffect(() => {
    ...
    window.api
      .invoke<string | null>(IPC_COMMANDS.ASSET_READ_BASE64, relativePath)
      .then(setDataUrl);
  }, [relativePath]);
  return dataUrl;
}
...
function SpriteCard({
  ...
  const displayName = entry.filename
    .replace(/\.[^.]+$/, "")
    .replace(/Modern_Office_Singles_32x32_/, "#");
...
function CharacterPlacementList(): JSX.Element {
  ...
  const handleQuickCreate = useCallback(async () => {
    ...
    const randomSkin =
      skins.length > 0 ? skins[Math.floor(Math.random() * skins.length)] : null;
...
function CharacterPlacementCard({
  character,
  setDraggingManifestId,
  onSkinChanged,
}: {
  character: Character;
  ...
  const preview = useSpritePreview(
    character.skin
      ? character.skin.startsWith("assets/")
  ...
  const handleSkinSelect = useCallback(
    async (path: string) => {
      await window.api.invoke(IPC_COMMANDS.CHARACTER_UPDATE, character.id, {
        skin: path,
      });
      onSkinChanged();
    },
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p
          className={`text-[10px] ${isPlaced ? "text-green-500" : "text-muted-foreground"}`}
        >
...
export function ItemPalette(): JSX.Element {
  ...
  const loadSprites = useCallback(async () => {
    setLoading(true);
    try {
      const entries = await window.api.invoke<SpriteEntry[]>(
        IPC_COMMANDS.ASSET_LIST,
      );
        ...
        for (let i = 0; i < entries.length; i += 20) {
          ...
          const results = await Promise.all(
            batch.map(async (entry) => {
              try {
                const data = await window.api.invoke<string | null>(
  ...
  const tileSprites = useMemo(
    () => allSprites.filter((s) => s.category === "background"),
    [allSprites],
  );
  const furnitureSprites = useMemo(
    () => allSprites.filter((s) => s.category === "furniture"),
    [allSprites],
  );
  ...
  const tileGroups = useMemo(
    () => groupByTheme(tileSprites),
    [groupByTheme, tileSprites],
  );
  const furnitureGroups = useMemo(
    () => groupByTheme(furnitureSprites),
    [groupByTheme, furnitureSprites],
  );

  const handleUpload = useCallback(async () => {
    const category: ItemCategory =
      activeTab === "background" ? "background" : "furniture";
    const relativePath = await window.api.invoke<string | null>(
      IPC_COMMANDS.ASSET_UPLOAD,
      category,
    );
    if (relativePath) {
      await loadSprites();
        ...
        {themeOrder.map((theme) => {
          ...
          return (
            <CollapsibleSection
              ...
              isBuiltin={entries[0]?.source === "builtin"}
              defaultOpen={theme !== "custom" || entries.length > 0}
            >
              {entries.length > 0 ? (
                <div className="grid grid-cols-3 gap-1.5">
                  {entries.map((entry) => (
                    <SpriteCard
                      ...
                      entry={entry}
                      dataUrl={spriteDataUrls.get(entry.relativePath)}
                    />
                  ))}
                </div>
              ) : (
